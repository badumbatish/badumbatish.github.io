...
the current function that the variable is in. Then we'd set `allocas[var_name] = var_alloca` in our symbol table.
...
, obtained via the symbol table mapping of `allocas`.

For the read case, in sammine, since everything's a value,  we'd just read the value at said
