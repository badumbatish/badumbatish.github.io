...
todo:
- Bold SS text
- Add live interval, live ranges explanation. When add, explain why
- Add first step of lin scan reg
## Prologue
...
- Map from one SSA value to a live interval.
- Optionally merge `compatible` live intervals together (this process is called coalescing).
...
we get our live intervals? How do we map SSA value to said interval? What mechanics helps us merge `compatible`? How is
MLIR helping here?
