...
For example, an `if-ne{:cpp}` is used to represent a `cmpi` instruction of two `mlir::Value{:cpp}`, and then depending on if they're equal or not, jump to a block.

I'll once again use the SSA property to resolve this issue.

Knowing that `mlir::Value{:cpp}` is SSA, upon a branching based on a predicate, we can query the defining op of an `mlir::Value{:cpp}` to obtain its boolean defining operation (step 1 in mlir). Then,  we proceed to pick the correct branching smali instruction. Since the nature of SSA is a value can only be defined once, a single value can not correspond to different boolean defining operation.
