...
The report is about 2500 words, or 10 minutes of reading. I also include a supporting section to recap different terminologies for unfamiliar readers.
...
- An input DEX file comprises a `ModuleOp{:cpp}`, which represent a class.
...
On the high level overview, we keep an `mlir::DenseMap{:cpp}` between the original CFG's basic blocks to a `BasicBlockDef{:cpp}`, this is means to replace the paper's `currentDef[variable][Block]{:cpp}`.
...
In the BasicBlockDef wraps another `mlir::DenseMap{:cpp}` of integers to `mlir::Value{:cpp}`, this is how we're accessing the `variable` in `currentDef[variable][Block]` from the paper's algorithm.
...
I'll once again use an `mlir::DenseMap{:cpp}` to resolve this issue; in the end, I provide some examples while another simpler method might not work.
