...
The report is about 3700 words, or 15 minutes of reading. I also include a supporting section to recap different terminologies for unfamiliar readers.
...
Introductory asides, in our library, we reuse two of the built-in dialects: `arith{:cpp}`, `cf{:cpp}`. We also extend upon this by creating new operations:
...
Fortunately with MLIR's helpful methods such as `llvm::dyn_cast<>(){:cpp}`, `llvm::isa<>(){:cpp}`, `mlir::BlockArgument{:cpp}`'s `getOwner()` and `mlir::Block{:cpp}`'s `isEntryBlock(){:cpp}`, it was relatively straightforward to resolve this.
...
Thus, if an `mlir::Value{:cpp}` is actually of `mlir::BlockArgument{:cpp}` and if it belongs to the entry block of a method, then it is a parameter, and will be inputted to `prc{:cpp}` and vice versa. I add the second requirement since a block argument can also come from different blocks that are divided by control flow instead.

#### Dealing with phis (block arguments)
...
In MLIR, we first obtain a boolean value from something like `arith{:cpp}`'s `cmpi{:cpp}`. Then we use this value to feed it
...
For example, an `if-ne{:cpp}` is used to represent a `cmplong` instruction of two virtual registers, and then depending on if they're equal or not, jump to a block.

I'll once again use an `mlir::DenseMap{:cpp}` to resolve this issue.

Knowing that `mlir::Value{:cpp}` can be hashed into `mlir::DenseMap{:cpp}`, and that it is SSA, we can make a one to one mapping between an `mlir::Value{:cpp}` and its boolean defining operation (step 1 in mlir). Then when we encounter a `cf.condbr{:cpp}`, we can look up the appropriate boolean defining operation of the predicate and pick the correct smali instruction.
