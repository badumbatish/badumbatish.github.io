...
Then it splits this source code string into different strings seperated by newlines, and construct a `std::vector{:cpp}
` of pairs of the running length of the concatenation of all the strings up to the current string and the current
...
allows us to print out the involving row).
