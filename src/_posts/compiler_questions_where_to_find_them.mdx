...
Follow-ups on leetcode questions were still the same: "what's the time complexity?"
#### Round 4
...
They said this could work. But another way was to perform randomized testing: pick a number randomly in a range and
test it.
...
refactor the
original
test to accept a callable instead (with `std::function{:cpp}`). I embarrassingly needed interviewer to remind me how to
...
difficult it is to
debug a JIT compiler instead, and the interviewer seemed satisfied.
...
not. If it is not linear in its complexity, what would you need to confirm/reject this hypothesis?"
