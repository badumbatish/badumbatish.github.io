...
#### Round 1
The pre-screen round (round 1) started off with [Leetcode 509 - Fibonacci](https://leetcode.
com/problems/fibonacci-number/description/)

Interviewer asked me to implement the naive, recursive approach. Then moved on to ask me how I would test them and
asked me
to come up with different ways to test this. They also asked me how to make sure the test wasn't just reusing the linear
...
skills we're looking for, but it's not there yet." I was thankful for the recruiter's transparency. Needless to say,
what
...
- "How does linear register allocation work?" - I have no hint for this question.
### Phở
...
Why is
that?" (Hint: something something vtables)
...
- "If, in testing, my implementation of an algorithm runs twice as slow when my input $n=200$ is twice bigger than my
$n=100$, does
that mean my implementation is linear in its complexity? Give your reasoning or counter examples on why it is or is
not. If it is not linear in its complexity, what would you do to confirm/reject this hypothesis?"
...
may complicate my achievements, but more importantly, they absolutely enlighten new grad readers a clear vision to
paint a
