---
title: 'Compiler (related) questions and how to find them'
date: 2025-4-17
recap: "Jasmine shares her new grad interview journey via some questions"
---

# Prologue

Hi everyone, how's everyone doing? It's been a while since I've written another blog. I'm doing good :) This May of
2025 I'm graduating and happy to annouce that I'm starting my employment at Igalia this June hacking on compilers :)

Igalia is a
"worker-owned, employee-run cooperative model consultancy focused on open source software" and I'm so happy I am
accepted and offered to work here. I'll be working remotely from work, hacking on compilers :) Besides the
...
- Ahmad: for chatting and being dumb with me through the latter half of recruitment cycle.
- Barbara: for practicing Leetcode with me (fall 2024).
- Sriram: For your reference.
- Iman: For referring me at xxx and yyy :) i'll always be grateful to you.
# Introduction

It's often discouraging to .... In this currently dire and over-competitive economy, once you apply for a role, it's
hard to get past the OA stage; once you get past the OA stage, it's
hard to get
past the resume
screen
stage; once you get
past the resume screen stage, it's hard to get past the interview stage. Companies are asking compiler questions that are often not
...
also won't be no behavioral questions mentioned.  I hope
everyone enjoys :)

## Pre-December 2024

During this time starting between Aug 2024 to December 2024, my mind was still fresh in the game. Limited interviews
from the previous year have frozen my leetcode and behavioral interview skills.
I had been doing
leetcode on the daily throughout the timeline; by the time the pre-December period ends, I would have done around 350
leetcode
problems.

Beginning of August 2024, I haven't taken any advanced compiler course nor have I been focused on my own toy
compiler project. The GSoc project for gccrs had just ended, and I was still trying to frame my résumé in
the best light for
potential employers.
...
After the code passes the test case, the interviewer asked how I can speed it up.
I answered by using cache on edges of the graph.
Interviewer then explained that cache is good but hinted that caching on the nodes of the graph is more efficient.
...
There were 2-3 follow up questions:

- What is the time complexity of this
- How do we make sure we don't waste time computing the combination sum that doesn't have an answer.
- Given a big list of element to sum up, and a large number n, and around 1000-2000 machines, on a high level, how
would you parallelize this?

#### Round 3

Interviewer starts with non-leetcode questions, then they transition into leetcode questions.

- Why hardware is important in faster inferrence of machine learning models?
...
Follow up on leetcode questions are still the same: whats the time complexity
#### Round 4
...
by the question a bit. But I've been hand-writing recursive descent parser in my compiler project so I kinda got the
general idea.
...
My resume's been much more refined with my graduate class
project.

Interviewing with a lot of stray companies made me more indifferent, more emboldened and wiser to
the recruiting process. Ghosting, generic answers doesn't bother me as much. Rejections just mean there are better
candidates than me, and they are a much better fit than me, and that I need to strive to better myself more.

I learned that in a competitive market, it is important to market oneself to the employers. Thus, I polished my
website and published a lot of blogs, and then posting them on LinkedIn, X, bluesky for visibility, meanwhile
advertising
in each blog article that I am actively looking for a full time compiler job.

It is important to advocate for yourself in an over-saturated market, whether it is through your resume or
general marketing. Please never forget that.
...
Another leetcode-styled question named "Bit-flip." I think companies really like leetcode questions.

Given a monochrome bitmap represented as a flat array of bytes, with one bit per pixel, flip it horizontally in-place.
The follow-up question is "if you have a lot of request, what kind of optimization/parallelization can you perform"

The problem was 3-fold. It tested if the interviewee could map from 2D array to 1D array. It then tested if
interviewee is comfortable with bit manipulation. Finally, it tested the interviewee ability to parallelize the problem.
#### Round 3
...
questions, all have managed to catch me off-guard in different ways.

#### Round 1

Given a sequence of matrix multiplication, optimize the order of matrix multiplication such that we use the least
...
This problem can be solved in $O(2^n)$ time naively or $O(n^3)$ time with dynamic programming. I rank it a leetcode
...
I have done the problem 2 or 3 times over, and am able to answer.
#### Round 3

Interviewer asked "how would you lower a convolution down to some specific architecture."

Interviewer also asked about my projects with GPU and AI. I didn't have much experience in the areas and thus could
not answer either problem. I'm still trying to improve in the area after feeling pretty defeated.
...
Interviewer asked me to implement the naive approach. Then moved on to ask me how would I test them and asked me
to come up with different ways to test this. They also asked me how to make sure the test isn't just reusing the linear
pattern: testing f(1), then f(2), then f(3) as the implementator can easily exploit the linear nature.
...
I proposed uin64_t in C++ and then they asked me "why not just use unsigned." I said that different platforms might
have different meaning for `unsigned`, and that "users use our fibonacci library over our competitor partly due to the
...
and I think that must have made the interviewer think I'm struggling. They directed me to have two variables, now named
`current` and `prev`.
...
Things slowly got more complicated. I need to test the iterative version as well so I then have to refactor the
original
test to accept a callable instead (with std::function). I embarrassingly needed interviewer to remind me how to
embed a function signature to std::function, as I didn't remember the old school way with function pointers in C++.
...
I then proposed that we used C++'s `std::map<int, int>` to handle this: binary search to the smaller and nearest cached
instance
...
The Interviewer seemed satisfied, and we ended this round of interview.
...
```kotlin
fun printArrayElements(arr: List<Int>) {
...
the jump assembly section might affect the execution speed of my CPU (hint: this depends on the branch predictor, cache
size of the
...
The interviewer asked me about the different phases of the compiler (front-end, middle-end, back-end).

The interviewer asked me about my toy compiler project, and what I plan to do next with it. I told them "I probably
would want to implement a type checker in." They then iterated on this, saying "ok how would you assign types in
your language initially." After a little bit of work, I told them "after the parsing and AST construction stage, I
would assign types, for example: a NumberExpr would probably get a `f64_t` type."

We moved on to talk about if-else type checking. They asked me to give actual pseudo code that type checks an
...
They then moved on to ask me how I would figure out that given two types A and B, find the type that is the
...
The Takoyaki company process is really different. They seemed really interested in trying to know me as a person,
...
the project has the word
"compiler" or "programming languages,", they're gonna ask in details each line that I wrote in the resume.

They also asked me that in my toy language, does it have error reporting? I reluctantly said no, and that's why I
incorporate that feature and wrote about it in this [article](what_after_kalei_location_debug).

There are also non-resume, non-leetcode questions:
...
- "on a high level, how would you debug/detect a memory bug" - Static analyzer and sanitizers.
- "how useful do you think AI is in your day to day workflow" - It's not good enough, but it's helpful for searching
for documentation.
- "described how you would debug an issue that popped up on your most recent commit" - Git bisect.
- "what do you like to do outside of coding, in a business setting, there may be other roles you can take on" -
Same as the `Sinigang ng hipon` interviewer in round 1, my answer is technical writings.
