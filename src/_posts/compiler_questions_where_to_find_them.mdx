...
might
have been too obvious, or I haven't touched it yet :P
...
on an application. All I could do was keep my head down and keep on grinding.
...
[Leetcode 40 - Combination Sum II](https://leetcode.com/problems/combination-sum-ii/) and
...
The interviewer provided me with the schematics for the untyped lambda calculus syntax and asked me to code up a
recursive
...
I was quite desolate and crushed. That week, each breath was shaky and heavy. A lot of hugs were needed.
...
I could feel some hope then, sending out applications and networking.
...
I could only come up with the naive solution.
...
needed throughout all the iterations. I'm glad I know vim pretty well to handle all of them.
...
For visualization, I typed something similar to this:
```cpp
...
LS of 2 types, with this in mind, how would you go on to check the LS of k types?" (Hint: Draw
out a
...
answers. It's clear to say their challenging questions perfectly blended between practicality, depth and theory.

Looking back, I think working more on my toy compiler could have helped a lot.
If I had
...
one of
our school's upper divisions in software-hardware architecture instead of randomized algorithms, answering
the
question on
assembly lowering could have been smoother. Whatever, we work with what we have :)
...
skills the team was looking for, but it's not there yet." I was grateful for the recruiter and the team's honesty.
Needless to say,
what
...
high frequency in questions in randomly different areas that were tailored specifically to my résumé.
...
As a result, they asked me a ton of questions about my résumé, about my experience in GSoc gccrs, about
...
questions
were asked in both rounds, the interviews at Takoyaki were still highly technical.
### Phở

I have a disclaimer to make. Phở is actually not a single company. But it's actually an amalgamation of different
companies' questions that were asked. In protecting companies' anonymity, I want to add all the relevant questions
...
facet? It is completely ok if the experience is not compiler-related."
...
- "Why do you think inlining and register allocation are really important in compiler optimization? It is ok to take
...
$n_1=100$, does
that mean my implementation is linear in its complexity? Can you provide your reasoning or counter-examples on why
it is
or is
...
compared to when my
input size was
$n_1=500$, does it imply that my implementation is $O(n^2)$?"
