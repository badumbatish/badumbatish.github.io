---
title: '[ONGOING] Building BRT.nvim - an atuin clone for my neovim workflow'
date: 8888-08-08
recap: "Jasmine builds a tool to speed up her development cycle"
---
# Introduction
...
# BRT.nvim
[BRT](https://github.com/badumbatish/brt.nvim) stands for Build, Run and Test and the plugin's description on GitHub is "Build, run, and test your projects with 3 keystrokes."
...
it would take me some brain cells to find the line dictating the file, and then I also have to
...
to the terminal to read the error message for that line. Wewww, even reading all this makes my head a bit dizzy.
...
Let's dive in to see how I solve the aforementioned problems in BRT.nvim

# Components
Now, building an [atuin](https://atuin.sh/) from pure scratch is going to be quite a task: self implementing fuzzy finding, and writing a database from scratch
...
## fzf-lua
[fzf-lua](https://github.com/ibhagwan/fzf-lua), in a few words, is "Improved fzf.vim written in lua."
...
Overall, the details docs and its ability to offload arguments to fzf itself means I can easily configure new keymaps as well as
search behaviors.
...
To initiate a fzf search inside neovim with fzf-lua, one can call `fzf_lua.exec_fzf(...)` and pass in the list of commands; imo, this is pretty straightforward.
...
For each command, I store the command itself, the command type (build, run, or test), duration (how long it run for), times inputted, last inputted, as well as the exit code.

Regarding the portability of (aka the ability to remember) these commands when a user move to a new machine (which is a thing that atuin offers),
...
There is always the fear that hackers will exploit said commands, but since the project only focuses on commonly run commands such as `ninja` or `npm` or `cmake` and
...
handle the ouput of stdout and stderr ourselves: combine them together and write to the log. However, doing so is quite laggy; when we're
...
and fall back to a more static progress report. By piping the output through tee itself, we'll be able to save a lot of processing power this way.

# Features
## BRTLog
As mentioned in the section above, when the user input a file, we would like to append `2>&1 | tee` at the end of the command to allow us to store the output as well for future revisions.
...
- how fast to change which underlying command `:make` will be using
...
Sensible behaviors from a plugin is quite hard to achieve, after all, how can you expect to write a plugin that acts
...
When a command is successful, I noticed that I don't really care about its output anymore, I made it so that BRT.nvim automatically
closes the terminal buffer and displays a successful message; seeing the green checkmark message saves me a few mental capacity
to type `:q` to close the terminal buffer as well as reading the terminal output to see if a command is successful or not.

Below is a screenshot showing what the green checkmark looks like
...
For roughly 2 days, I've run all `ninja -C build`-related commands about 100 times or more. With how tight my development cycle has become, I would say the plugin has already paid
dividends on itself :)

# Ending

I hope you've enjoyed reading through how a clone tool of atuin came to be inside neovim itself :) I know I've really enjoyed
developing the plugins as well as writing this article :) Please feel free to give them a try at https://github.com/badumbatish/brt.nvim.
